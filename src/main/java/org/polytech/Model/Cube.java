...
public class Cube {
     ...
     */
    // Закрашивает куб стандартными цветами
    public Cube() {
        //Up, Front Row
    ...
    public String finishWhiteLayer() {
        ...
        String adjustedMoves1 = insertCornersInU();
        String adjustedMoves2 = insertMisorientedCorners();
        if (adjustedMoves1.equals("failed") || adjustedMoves2.equals("failed"))
            return "failed";
        moves.append(adjustedMoves1);
        moves.append(adjustedMoves2);
        ...
        while(whiteCornerinU()) {
            ...
            adjustedMoves1 = insertCornersInU();
            adjustedMoves2 = insertMisorientedCorners();
            if (adjustedMoves1.equals("failed") || adjustedMoves2.equals("failed"))
                return "failed";
            moves.append(adjustedMoves1);
            moves.append(adjustedMoves2);
    ...
    public String permuteLastLayer() {
        ...
        if (!solved())
            moves += performMoves("D L2 B2 F2 R2 U' L2 B2 F2 R2");
        if (!solved())
            moves += performMoves("D L2 B2 F2 R2 U' L2 B2 F2 R2");
        if (!solved())
            moves += performMoves("D L2 B2 F2 R2 U' L2 B2 F2 R2");
